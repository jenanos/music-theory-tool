...
export function ChordDisplay({ chordLine, degreeLine, className, onClick, onChordClick }: ChordDisplayProps) {
    ...
    if (!chordLine.trim()) {
        return (
            <div
                onClick={onClick}
                className={`min-h-[80px] w-full rounded-md border-2 border-dashed border-border bg-muted p-4 text-center text-sm text-muted-foreground hover:border-primary/50 hover:bg-muted/80 ${className}`}
            >
    ...
    return (
        <div
            onClick={onClick}
            className={`flex flex-col gap-2 rounded-md bg-muted p-3 min-h-[60px] cursor-pointer hover:bg-muted/80 ring-1 ring-border ${className}`}
        >
                    ...
                    {row.chords.length > 0 ? (
                        row.chords.map((chord, chordIndex) => (
                            <div
                                ...
                                className={`flex flex-col gap-1 items-center justify-center p-2 rounded-lg bg-card shadow-sm ring-1 ring-border transition-transform hover:-translate-y-0.5 hover:shadow-md ${onChordClick ? 'cursor-pointer hover:ring-primary/50' : ''}`}
                            >
