...
describe("transposeLickData", () => {
  ...
  it("transposes between modal contexts by parent scale", () => {
    const data: LickData = {
      events: [{ string: 1, fret: 5, duration: 8 }],
    };

    expect(
      transposeLickData(data, "C ionian", "D dorian").data.events[0],
    ).toEqual({
      string: 1,
      fret: 5,
      duration: 8,
    ...
    expect(
      transposeLickData(data, "C ionian", "E dorian").data.events[0],
    ).toEqual({
      string: 1,
      fret: 7,
      duration: 8,
    ...
    expect(
      transposeLickData(data, "D dorian", "E dorian").data.events[0],
    ).toEqual({
      string: 1,
      fret: 7,
      duration: 8,
  ...
  it("warns for negative frets after downward transposition", () => {
    const result = transposeLickData(
      { events: [{ string: 1, fret: 1, duration: 4 }] },
      "D",
      "C",
    );
...
describe("lickDataToAlphaTex", () => {
  ...
  it("emits best-effort AlphaTex technique markers", () => {
    ...
    expect(alphaTex).toContain("5.3{sl}.8 7.3.8");
    expect(alphaTex).toContain("5.2{h}.8 8.2.8");
    expect(alphaTex).toContain("8.2{h}.8 5.2.8");
    expect(alphaTex).toContain("7.1{b (0 4) v}.4");
    expect(alphaTex).toContain("7.1{t}.4");
    expect(alphaTex).toContain("7.1{g}.4");
  });

  it("renders dotted duration imports without invalid AlphaTex", () => {
    const warnings = validateLickData({
      events: [
        {
          string: 1,
    ...
    expect(warnings).toEqual([
      'Event 0: dotted duration "4." is rendered as 4 in v1',
    ]);
    expect(alphaTex).toContain("18.1{g}.4");
  });
});
