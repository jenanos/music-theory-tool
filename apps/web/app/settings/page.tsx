...
import { USER_THEME_OPTIONS, type UserTheme } from "../lib/theme";
...
export default function SettingsPage() {
  const { user, isLoading, enabledPages, theme, updateTheme } = useAuth();
  ...
  const handleThemeChange = async (nextTheme: UserTheme) => {
    ...
    try {
      ...
      setThemeMessage("Tema lagret.");
    } catch (error) {
      setThemeMessage(
        error instanceof Error ? error.message : "Kunne ikke lagre tema.",
      );
    } finally {
    ...
    <div className="mx-auto max-w-2xl px-4 py-8 md:px-6">
      ...
      <section className="mt-6 rounded-xl border border-border bg-card/60 backdrop-blur-xl p-6">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Velg mellom dagens Tynn Is-tema eller mer tradisjonell light mode og
          dark mode.
        </p>
        ...
        {themeMessage && (
          <p className="mt-4 text-sm text-muted-foreground">{themeMessage}</p>
        )}
      </section>
      ...
      {/* Groups */}
      <section className="mt-6 rounded-xl border border-border bg-card/60 backdrop-blur-xl p-6">
        <h2 className="text-lg font-semibold text-foreground mb-4">Grupper</h2>
        {!groupsLoaded ? (
