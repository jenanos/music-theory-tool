...
export const Fretboard: React.FC<FretboardProps> = ({
    ...
    return (
        <div className={cn("relative w-full overflow-hidden select-none p-4 flex justify-center", className)}>
            <div className="w-full max-w-[400px] h-[75svh] min-h-[400px] md:max-w-none md:min-h-0 md:h-[300px] md:w-[800px] flex md:flex-row flex-col bg-card rounded-lg shadow-2xl border-2 border-border relative">
                {/* Nut */}
                <div className="
                    absolute z-20 bg-muted border-border
                ...
                <div className="flex flex-1 flex-col md:flex-row pt-8 pl-0 md:pt-0 md:pl-8 relative z-0">
                    {Array.from({ length: FRETS }).map((_, i) => (
                        <div
                            key={i}
                            className="
                                flex-1 border-muted-foreground/30 relative flex items-center justify-center group
                                border-b md:border-r md:border-b-0
                                w-full md:h-full
                            "
                        >
                            {DOTS.includes(i + 1) && (
                                <div className="absolute w-4 h-4 rounded-full bg-muted-foreground/20 backdrop-blur-sm" />
                            )}
                            <div className="
                                absolute text-xs text-muted-foreground font-mono
